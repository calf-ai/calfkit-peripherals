...
- **`ModelRetry`** — upstream retry-signal exception (`pydantic_ai.exceptions`). **Replaced**
  by the neutral first-party `WebFetchError` (`calfkit_tools.web_fetch.results`). The engine
...
- **`BinaryContent`** — upstream binary content type (`pydantic_ai.messages`). **Replaced** by
  ...
  (`calfkit_tools.web_fetch.results`, §6 — at the engine/`node` boundary, NOT in `_vendor/`).
