# web_fetch (calfkit-tools source)

The `web_fetch` source of **calfkit-tools** — vendoring the **SSRF-safe `web_fetch` tool**
from [pydantic/pydantic-ai](https://github.com/pydantic/pydantic-ai) (MIT).
...
- `src/calfkit_tools/web_fetch/_vendor/` — the vendored upstream `_ssrf.py` + fetch
  engine (import-rewritten, self-contained: stdlib + `httpx` + `markdownify`).
- `src/calfkit_tools/web_fetch/node/` — calfkit Kafka adapter + tool wrapper (your code).
- `tests/web_fetch/` — vendored upstream regression suites (`test_ssrf.py`, `test_web_fetch.py`) +
  adapter tests.
- `vendor/web_fetch/patches/` — re-appliable functional changes to vendored code (the §5 streaming restructure).
- `vendor/web_fetch/` (this dir) — provenance only (`LICENSE`, `METADATA.yaml`, `NODE.md`).
...
calfkit tool node: `calfkit_tools.web_fetch.node.web_fetch` is a deployable
