...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    # (`calfkit_tools.hermes._vendor.tools`) rather than the literal `tools.` — the
...
class ToolRegistry:
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            if entry.is_async:
                from calfkit_tools.hermes._shims.model_tools import _run_async
            ...
            try:
                from calfkit_tools.hermes._shims.model_tools import _sanitize_tool_error
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from calfkit_tools.hermes._vendor.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
