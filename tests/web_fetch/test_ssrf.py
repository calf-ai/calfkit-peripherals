...
from calfkit_tools.web_fetch._vendor._ssrf import (
...
def mock_dns(monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    ...
    mock = AsyncMock()
    monkeypatch.setattr('calfkit_tools.web_fetch._vendor._ssrf.run_in_executor', mock)
    return mock
...
def mock_ssrf_client(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr('calfkit_tools.web_fetch._vendor._ssrf.create_async_http_client', factory_wrapper)
    return mock
...
class TestSafeDownload:
    ...
    async def test_safe_download_closes_http_client(self, mock_dns: AsyncMock, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr('calfkit_tools.web_fetch._vendor._ssrf.create_async_http_client', tracking_create)
