# Vendored from NousResearch/hermes-agent @ 5a36f76 (MIT) -- tests/tools/test_approval_plugin_hooks.py
...
def isolated_session(monkeypatch, tmp_path):
    ...
    import calfkit_tools.hermes._vendor.tools.approval as _am
...
class TestCliPathFiresHooks:
    ...
    def test_pre_and_post_fire_with_expected_kwargs(
        ...
        with patch("calfkit_tools.hermes._shims.hermes_cli.plugins.invoke_hook", side_effect=fake_invoke_hook):
    ...
    def test_plugin_hook_crash_does_not_break_approval(
        ...
        with patch("calfkit_tools.hermes._shims.hermes_cli.plugins.invoke_hook", side_effect=boom):
            result = check_all_command_guards(
