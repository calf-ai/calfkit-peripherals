# Vendored from NousResearch/hermes-agent @ 5a36f76 (MIT) -- tests/tools/test_process_registry.py
...
from calfkit_tools.hermes._vendor.tools.environments.local import _HERMES_PROVIDER_ENV_FORCE_PREFIX
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(os.environ, {
            ...
            patch("calfkit_tools.hermes._vendor.tools.process_registry._find_shell", return_value="/bin/bash"), \
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        with patch("calfkit_tools.hermes._vendor.tools.process_registry.threading.Thread", return_value=fake_thread), \
            patch.object(registry, "_write_checkpoint"):
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("calfkit_tools.hermes._vendor.tools.process_registry.time.sleep", return_value=None), \
...
class TestPopenLeakOnSetupFailure:
    ...
    def test_popen_killed_when_thread_creation_fails(self, registry):
        ...
        with patch("calfkit_tools.hermes._vendor.tools.process_registry._find_shell", return_value="/bin/bash"), \
...
class TestCheckpoint:
    def test_write_checkpoint(self, registry, tmp_path):
        with patch("calfkit_tools.hermes._vendor.tools.process_registry.CHECKPOINT_PATH", tmp_path / "procs.json"):
            s = _make_session()
            registry._running[s.id] = s
    ...
    def test_recover_no_file(self, registry, tmp_path):
        with patch("calfkit_tools.hermes._vendor.tools.process_registry.CHECKPOINT_PATH", tmp_path / "missing.json"):
    ...
    def test_recover_dead_pid(self, registry, tmp_path):
        ...
        with patch("calfkit_tools.hermes._vendor.tools.process_registry.CHECKPOINT_PATH", checkpoint):
...
class TestKillProcess:
    ...
    def test_kill_detached_session_uses_host_pid(self, registry):
        ...
        try:
            ...
            with patch("calfkit_tools.hermes._shims.gateway.status._pid_exists", return_value=True), \
...
class TestProcessToolHandler:
    def test_list_action(self):
        from calfkit_tools.hermes._vendor.tools.process_registry import _handle_process
...
from calfkit_tools.hermes._vendor.tools.process_registry import format_process_notification
...
class TestTerminateHostPidWindows:
    ...
    def test_windows_invokes_taskkill_with_tree_and_force_flags(self, monkeypatch):
        ...
        from calfkit_tools.hermes._vendor.tools import process_registry as pr
