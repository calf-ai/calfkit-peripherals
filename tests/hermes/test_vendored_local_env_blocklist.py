# Vendored from NousResearch/hermes-agent @ 5a36f76 (MIT) -- tests/tools/test_local_env_blocklist.py
...
def _run_with_env(extra_os_env=None, self_env=None):
    ...
    with patch("calfkit_tools.hermes._vendor.tools.environments.local._find_bash", return_value="/bin/bash"), \
         patch("subprocess.Popen", side_effect=_make_fake_popen(captured)), \
         patch("calfkit_tools.hermes._vendor.tools.terminal_tool._interrupt_event", fake_interrupt), \
...
class TestSanePathIncludesHomebrew:
    ...
    def test_sane_path_includes_homebrew_bin(self):
        from calfkit_tools.hermes._vendor.tools.environments.local import _SANE_PATH
    ...
    def test_make_run_env_appends_homebrew_on_minimal_path(self):
        ...
        from calfkit_tools.hermes._vendor.tools.environments.local import _make_run_env
