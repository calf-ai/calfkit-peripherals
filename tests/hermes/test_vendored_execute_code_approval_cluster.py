# Vendored from NousResearch/hermes-agent @ 5a36f76 (MIT) -- tests/tools/test_execute_code_approval_cluster.py
...
from calfkit_tools.hermes._vendor.tools import approval as A
...
def test_helper_propagates_contextvar_and_approval_callback():
    from calfkit_tools.hermes._vendor.tools import terminal_tool as TT
...
def test_both_rpc_threads_use_propagation_helper():
    ...
    silently returns."""
    import inspect
    import calfkit_tools.hermes._vendor.tools.code_execution_tool as cet
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    from calfkit_tools.hermes._vendor.tools.code_execution_tool import _scrub_child_env
...
def test_env_scrub_logs_dropped_hermes_vars(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="calfkit_tools.hermes._vendor.tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)
