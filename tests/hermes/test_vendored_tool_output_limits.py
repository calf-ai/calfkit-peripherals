# Vendored from NousResearch/hermes-agent @ 5a36f76 (MIT) -- tests/tools/test_tool_output_limits.py
...
from calfkit_tools.hermes._vendor.tools import tool_output_limits as tol
...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_config_missing(self):
        with patch("calfkit_tools.hermes._shims.hermes_cli.config.load_config", return_value={}):
    ...
    def test_get_limits_returns_defaults_when_config_not_a_dict(self):
        ...
        with patch("calfkit_tools.hermes._shims.hermes_cli.config.load_config", return_value="not a dict"):
            limits = tol.get_tool_output_limits()
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("calfkit_tools.hermes._shims.hermes_cli.config.load_config", side_effect=_boom):
            limits = tol.get_tool_output_limits()
...
class TestOverrides:
    def test_user_config_overrides_all_three(self):
        ...
        with patch("calfkit_tools.hermes._shims.hermes_cli.config.load_config", return_value=cfg):
            limits = tol.get_tool_output_limits()
        assert limits == {
            "max_bytes": 100_000,
            "max_lines": 5000,
...
class TestDefaultConfigHasSection:
    ...
    def test_default_config_contains_tool_output_section(self):
        from calfkit_tools.hermes._shims.hermes_cli.config import DEFAULT_CONFIG
...
class TestIntegrationReadPagination:
    ...
    def test_pagination_limit_clamped_by_config_value(self):
        from calfkit_tools.hermes._vendor.tools.file_operations import normalize_read_pagination
