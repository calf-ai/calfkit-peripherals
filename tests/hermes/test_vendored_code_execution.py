# Vendored from NousResearch/hermes-agent @ 5a36f76 (MIT) -- tests/tools/test_code_execution.py
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        with patch("calfkit_tools.hermes._vendor.tools.code_execution_tool._load_config", return_value={"timeout": 30, "max_tool_calls": 5}), \
             patch("calfkit_tools.hermes._vendor.tools.code_execution_tool._get_or_create_env", return_value=(env, "ssh")), \
             patch("calfkit_tools.hermes._vendor.tools.code_execution_tool._ship_file_to_remote"), \
             patch("calfkit_tools.hermes._vendor.tools.code_execution_tool.threading.Thread", return_value=fake_thread):
...
class TestExecuteCode(unittest.TestCase):
    ...
    def _run(self, code, enabled_tools=None):
        ...
        with patch("calfkit_tools.hermes._vendor.tools.code_execution_tool._rpc_server_loop") as mock_rpc:
            # Use real execution but mock the tool dispatcher
            pass
        # Actually run with full integration, mocking at the model_tools level
        with patch("calfkit_tools.hermes._shims.model_tools.handle_function_call", side_effect=_mock_handle_function_call):
            result = execute_code(
                code=code,
                task_id="test-task",
...
'''
        ...
        with patch("calfkit_tools.hermes._shims.model_tools.handle_function_call", side_effect=slow_mock):
            raw = execute_code(
                code=code,
...
"""
    ...
    def test_timeout_enforcement(self):
        ...
        with patch("calfkit_tools.hermes._shims.model_tools.handle_function_call", side_effect=_mock_handle_function_call):
            ...
            with patch("calfkit_tools.hermes._vendor.tools.code_execution_tool._load_config", return_value={"timeout": 2, "max_tool_calls": 50}):
...
class TestStubSchemaDrift(unittest.TestCase):
    ...
    def test_stubs_cover_all_schema_params(self):
        ...
        import re
        from calfkit_tools.hermes._vendor.tools.code_execution_tool import _TOOL_STUBS

        # Import the registry and trigger tool registration
        from calfkit_tools.hermes._vendor.tools.registry import registry
        import calfkit_tools.hermes._vendor.tools.file_tools  # noqa: F401 - registers read_file, write_file, patch, search_files
        import calfkit_tools.hermes._vendor.tools.web_tools  # noqa: F401 - registers web_search, web_extract
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def _get_child_env(self, extra_env=None):
        ...
        try:
            ...
            with patch("calfkit_tools.hermes._shims.model_tools.handle_function_call", return_value='{}'), \
                 patch("calfkit_tools.hermes._vendor.tools.code_execution_tool._load_config",
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_windows_returns_error(self):
        ...
        with patch("calfkit_tools.hermes._vendor.tools.code_execution_tool.SANDBOX_AVAILABLE", False):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        with patch("calfkit_tools.hermes._shims.model_tools.handle_function_call",
...
class TestLoadConfig(unittest.TestCase):
    def test_returns_empty_dict_when_cli_config_unavailable(self):
        from calfkit_tools.hermes._vendor.tools.code_execution_tool import _load_config
    ...
    def test_returns_code_execution_section(self):
        from calfkit_tools.hermes._vendor.tools.code_execution_tool import _load_config
        with patch("calfkit_tools.hermes._shims.hermes_cli.config.read_raw_config",
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("calfkit_tools.hermes._shims.hermes_cli.config.read_raw_config", return_value={}):
            result = _load_config()
...
class TestInterruptHandling(unittest.TestCase):
    def test_interrupt_event_stops_execution(self):
        ...
        from calfkit_tools.hermes._vendor.tools.interrupt import set_interrupt
