# Vendored from NousResearch/hermes-agent @ 5a36f76 (MIT) -- tests/tools/test_url_safety.py
...
from calfkit_tools.hermes._vendor.tools.url_safety import (
    is_safe_url,
    async_is_safe_url,
...
class TestIsSafeUrl:
    ...
    def test_unexpected_error_fails_closed(self):
        ...
        with patch("calfkit_tools.hermes._vendor.tools.url_safety.urlparse", side_effect=ValueError("bad url")):
...
class TestAsyncIsSafeUrl:
    ...
    # Port note: upstream uses @pytest.mark.asyncio; driven via asyncio.run here to
    # avoid adding pytest-asyncio (matches the repo's existing async-test style).
    def test_public_url_allowed(self):
        with patch("socket.getaddrinfo", return_value=[
            (2, 1, 6, "", ("93.184.216.34", 0)),
        ]):
            assert asyncio.run(async_is_safe_url("https://example.com/x")) is True

    def test_localhost_blocked(self):
        with patch("socket.getaddrinfo", return_value=[
            (2, 1, 6, "", ("127.0.0.1", 0)),
        ]):
            assert asyncio.run(async_is_safe_url("http://localhost:8080/")) is False
...
class TestGlobalAllowPrivateUrls:
    ...
    def test_default_is_false(self, monkeypatch):
        ...
        with patch("calfkit_tools.hermes._shims.hermes_cli.config.read_raw_config", side_effect=Exception("no config")):
    ...
    def test_config_security_section(self, monkeypatch):
        ...
        with patch("calfkit_tools.hermes._shims.hermes_cli.config.read_raw_config", return_value=cfg):
            assert _global_allow_private_urls() is True
